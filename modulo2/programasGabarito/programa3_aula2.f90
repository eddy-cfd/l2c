program manning_exemplo

! Desejamos escrever um programa que resolva a raiz da seguinte equação:
! f(x) = (0.471405*(20.0*x)**(5.0/3.0))/((20.0 + 2.0*x)**(2.0/3.0)) - 5.0
...
xm = xs - (fs*(xi-xs))/(fi - fs) ! Valor da raiz antes da atualização do intervalo
...
function funcao(x) result(f)
   real, intent(in) :: x
   real f
   f = (0.471405*(20.0*x)**(5.0/3.0))/((20.0 + 2.0*x)**(2.0/3.0)) - 5.0
end function funcao


end program manning_exemplo
